...
- Решения, которые раньше жили в `StatusController` без тестов, вынесены в `Sources/Model` и проверяются модельной проверкой CI: `SessionBoard` (жизненный цикл сессий: перечитывание файлов, reap, звонок, «нужен ты», имена-двойники, главная сессия), `LimitsBoard` (какие лимиты показывают иконка и полоса), `Provider` (чем различаются агенты: имя, значок, папка состояния, файл настроек), `BackendCommand` (слова командной строки `mcpbar.py`). Контроллер читает диск и исполняет то, что они вернули; новое правило такого рода кладётся туда же, а не в контроллер.
- Все запуски `mcpbar.py` идут через `spawnBackend()` в `main.swift`: он пишет в лог stderr и код выхода любого неуспешного запуска. Второго пути запуска с выводом в `/dev/null` не заводить.
...
| `limits.json` | `capture_limits` в `hooks/statusline.py` и команда `limits` в `scripts/mcpbar.py` (двойной писатель; совпадение записей держит тест `LimitsWriters` в `tests/test_statusline.py`, образец для Swift — `build/seam/limits.json`) |
