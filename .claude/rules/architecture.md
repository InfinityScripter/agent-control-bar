...
| `~/.codex/hooks.json` | `hooks/install.js` (мерж, чужие хуки не трогает) и `hooks/uninstall.js` (снятие только при полном удалении, не при `--hooks-only`). Доверие к хукам (`[hooks.state]` в `config.toml`) выдаёт только пользователь в самом Codex — хэш не подделывать |
